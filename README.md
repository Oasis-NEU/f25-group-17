...
## Local cache  
The local cache generated by the catalogAPI module will create folder by term code. Each folder includes raw course data, course meeting times data, and room schedules data. The raw course data is the data taken directly from banner. The course meeting times data is a cleaned up version of the raw data. The room schedules data is a processed version of the course meeting times to show the schedule of each room.
**To obtain a local copy of the cache, run the following:**  
```bash
cd .\catalogAPI\src\
node .\main.js
```  
