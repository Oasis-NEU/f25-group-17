...
export default function Signup() {
  ...
  const [isLoaded, setIsLoaded] = React.useState(false); // Flag to track if initial load is complete

  // Auto-save form data to localStorage whenever it changes (only after initial load)
  React.useEffect(() => {
    ...
    console.log("Auto-saved form data to localStorage:", dataToSave);
  }, [formData, courses, isLoaded]);
  ...
  // Fetch saved courses on component mount - from localStorage
  React.useEffect(() => {
    try {
      ...
      // Mark load as complete to enable auto-save
      setIsLoaded(true);
      console.log("Component load complete, auto-save enabled");
    } catch (err) {
      console.error("Error loading from localStorage:", err);
    }
  }, []);

  // Ensure data persists when navigating away
  React.useEffect(() => {
    const saveBeforeLeave = () => {
      try {
        ...
        localStorage.setItem("signupFormData", JSON.stringify(dataToSave));
        console.log("Data saved before navigation:", dataToSave);
      } catch (err) {
        ...
        <div className="relative z-10 w-full max-w-2xl mx-auto p-px rounded-3xl bg-linear-to-br from-red-600/30 via-gray-700/20 to-red-900/30">
          <div className="relative bg-linear-to-br from-black-900/90 via-gray-800/90 to-black-700/90 rounded-3xl p-12 backdrop-blur-sm shadow-[0_0_100px_rgba(15,23,42,0.8),0_0_50px_rgba(30,41,59,0.6),inset_0_0_60px_rgba(15,23,42,0.3)]">
            ...
            <div className="flex flex-col w-full gap-6">
              ...
              {/* Courses Section - Display saved courses or prompt to add */}
              <div>
                <label className="block text-red-400 text-sm font-semibold mb-2">
                  Courses
                </label>
                ...
                {courses.length > 0 && (
                  <div className="mb-3 flex flex-wrap gap-2">
                    {courses.map((course, idx) => (
                      <span key={`course-${idx}`} className="px-3 py-1 bg-red-600/20 border border-red-600/50 text-red-300 text-sm rounded-full">
                        {course}
                      </span>
                    ))}
                  </div>
